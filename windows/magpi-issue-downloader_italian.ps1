...
# VERSION=0.1.0
# USAGE="Usage: ./magpi-issue-downloader-italian.ps1"
...
if( -Not (Test-Path -Path "italian_issues" ) )
{
    New-Item -ItemType directory -Path "italian_issues"
}
...
while ($i -le $issues)
{
	$filePattern = "";
	if ($i -lt 10) {
		$filePattern = "MagPi_Mini_Italian_0$i.pdf"
	} else {
		$filePattern = "MagPi_Mini_Italian_$i.pdf"
	}
	...
	try
	{
		...
		$web.DownloadFile($fileUrl, "$PSScriptRoot\italian_issues\$filePattern")
	} Catch
	{
